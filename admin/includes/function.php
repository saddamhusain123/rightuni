...
function adminUser($username, $password){
    ...
    $sql = "SELECT id,username FROM admin where username = '".$username."' and password = '".password_hash($password, PASSWORD_DEFAULT)."'";       
   ...
   function checkSignSalt($data_info){
        ...
        if($data_arr['package_name']==PACKAGE_NAME){
            if($data_arr['sign'] == '' && $data_arr['salt'] == '' ){
                ...
                $set['ALL_IN_ONE_NEWS'][] = array("success" => -1, "MSG" => "Invalid sign salt.");
                ...
                if((isset($data_arr['user_id']) || $data_arr['user_id']!='') AND $data_arr['user_id']!=0){
                  ...
                  if(mysqli_num_rows($res) > 0){
                      ...
                      $sql="SELECT * FROM active_log WHERE `user_id`='$user_id'";
                      $res=mysqli_query($mysqli, $sql);
                      ...
                      if(mysqli_num_rows($res) == 0){
                          ...
                          $qry = Insert('active_log',$data_log);
                          ...
                          $update=Update('active_log', $data_log, "WHERE user_id = '$user_id'");  
                      }
                      ...
                      $set['ALL_IN_ONE_NEWS'][] = array("success" => -2, "MSG" => "Your account is deactived or deleted by admin !");   
                      header( 'Content-Type: application/json; charset=utf-8' );
                      echo $val= str_replace('\\/', '/', json_encode($set,JSON_UNESCAPED_UNICODE | JSON_PRETTY_PRINT));
                      exit(); 
                  }
             ...
             //$data['data'] = array("success" => -1, "msg" => "Invalid sign salt.");
            $set['ALL_IN_ONE_NEWS'][] = array("success" => -1, "MSG" => "Invalid package name");   
            header( 'Content-Type: application/json; charset=utf-8' );
            echo $val= str_replace('\\/', '/', json_encode($set,JSON_UNESCAPED_UNICODE | JSON_PRETTY_PRINT));
            exit();  
        }
